...
const NavBar: FC<Props> = ({ className }) => {
    ...
    return (
        <main className="w-full md:h-40 h-scree flex justify-center items-center font-sans md:p-10"> 
            {/* Desktop Navigation */}
            <section className={`${className} w-full h-20 max-w-screen-xl full-hd:max-w-screen-2xl justify-between items-center rounded-md p-5 hidden md:flex`}>
                <div>
                ...
                <div>
                    <button className="px-4 py-2 bg-midBlue text-white rounded-md">Book Now</button>
                </div>
            </section>
                    ...
                    <nav className=" bg-white absolute w-full overflow-hidden p-5">
                        <ul className="flex flex-col space-y-4 text-darkBlue cursor-pointer pb-10">
                            {Object.keys(paths).map((item) => (
                                <li
                                    key={item}
                                    className={active === item ? 'text-darkBlue font-bold' : 'border-b border-gray border-opacity-40'}
                                    onClick={() => handleClick(item)} // Set active on click
                                >
